 ...
 */
public class XPathTimeRangeQueryBuilder implements XPathVisitor, XPathTreeConstants {
     ...
     */
    private XPathTimeRangeQueryBuilder(String statement, NamespaceResolver resolver)
    ...
    public static QueryRootNode createQuery(String statement,
            ...
            throws InvalidQueryException {
        QueryRootNode root = new XPathTimeRangeQueryBuilder(statement, resolver).getRootNode();
        return root;
    }
    ...
    private QueryNode createFunction(SimpleNode node, QueryNode queryNode) {
        ...
        try {
            if (FN_NOT.toJCRName(resolver).equals(fName)
                ...
                if (node.jjtGetNumChildren() == 3) {
                    if (queryNode instanceof NAryQueryNode) {
                        SimpleNode period = (SimpleNode) node.jjtGetChild(2).jjtGetChild(0);
                        if (period.getId() == JJTSTRINGLITERAL) {
                            TimeRangeQueryNode timerange = new TimeRangeQueryNode(queryNode,
                            ...
                            exceptions.add(new InvalidQueryException("Wrong argument types for jcr:timerange"));
                        }
                    }
                } else {
                    // wrong number of arguments
                    exceptions.add(new InvalidQueryException("Wrong number of arguments for jcr:timerange"));
                }
            } else {
