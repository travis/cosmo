if(!dojo._hasResource['cosmo.view.cal.canvas']){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource['cosmo.view.cal.canvas'] = true;
/*
...
cosmo.view.cal.canvas = new function () {
    ...
    this.handleSelectionChange = function (e, id, discardUnsavedChanges) {
        ...
        if ((!origSelection) || (origSelection.id != item.id)) {
            ...
            var f = function () {
                dojo.publish('cosmo:calSetSelected', [{ 'data': item }]);
            }
            setTimeout(f, 0);
        }
