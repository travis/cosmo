...
<!-- Content Stylesheet for "cosmo-docs" Documentation -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param    name="home-name"        select="'Cosmo'"/>
  <xsl:param    name="home-href"        select="'http://cosmo.osafoundation.org/'"/>
  <xsl:param    name="home-logo"        select="'/cosmo_logo.gif'"/>
  <xsl:param    name="printer-logo"     select="'/printer.gif'"/>
  <xsl:param    name="osaf-logo"        select="'/osaf_logo_sm.gif'"/>
  <xsl:param    name="relative-path"    select="'.'"/>
  <xsl:param    name="void-image"       select="'/void.gif'"/>
  <xsl:param    name="project-menu"     select="'menu'"/>
  <xsl:param    name="standalone"       select="''"/>
  <xsl:param    name="buglink"          select="'https://bugzilla.osafoundation.org/'"/>

  <!-- Defined variables (non-overrideable) -->
  <xsl:variable name="body-bg"          select="'#ffffff'"/>
  <xsl:variable name="body-fg"          select="'#000000'"/>
  <xsl:variable name="body-link"        select="'#525D76'"/>
  <xsl:variable name="banner-bg"        select="'#6699ff'"/>
  <xsl:variable name="banner-fg"        select="'#ffffff'"/>
  <xsl:variable name="sub-banner-bg"    select="'#6699ff'"/>
  ...
  <xsl:variable name="project"
    ...
    <table border="0" width="100%" cellspacing="0">
      ...
      <tr>
        ...
        <td>
          <xsl:comment>OSAF LOGO</xsl:comment>
          <xsl:variable name="src">
            <xsl:value-of select="$relative-path"/><xsl:value-of select="$osaf-logo"/>
          </xsl:variable>
