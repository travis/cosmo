...
dojo.provide("cosmo.util.tests.base64");
...
doh.register("cosmo.util.tests.base64", [
    function unicode(){
        for (var i in unicodeStrings){
            var b64 = cosmo.util.encoding.toBase64(unicodeStrings[i]);
            jum.assertEquals(
                unicodeStrings[i] + "(index " + i + ")" + " didn't encode correctly",
                base64Strings[i],
                b64
                );
        }
