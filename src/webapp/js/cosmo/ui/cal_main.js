...
var Cal = new function() {
    ...
    this.updateAllEventsDisplayed = function() {
        return (Cal.sortAndClearEvents() &&
            Cal.checkConflicts() && Cal.stackUntimed() && Cal.placeBlocks());
    }
    /**
     ...
     */
    this.sortAndClearEvents = function() {
        var evReg = Cal.eventRegistry;
        var ev = null;

        // Inner functions -- these will likely leak like a mofo in IE
        // ------------------
        ...
        var clearProps = function(key, val) {
            ev = val;
            ev.beforeConflicts = [];
            ev.afterConflicts = [];
            ev.conflictDepth = 0;
        ...
        // Clear out the matrix of untimed event placements
        Cal.clearAllDayMatrix();

        return true;
    };
    /**
     * Set up multidimensional array for sparse matrix for
     * placement of untimed events
     */
    this.clearAllDayMatrix = function() {
        this.allDayMatrix = [[], [], [], [], [], [], []];
    };
    /**
    ...
    this.checkConflicts = function() {
        ...
        for (var i = 0; i < evReg.length; i++) {
            this.checkConflictAtPos(i);
        }

        // Calc the actual indent level for each event
        evReg.each(Cal.calcConflictDepth);

        // Cal the full width of all the overlapped events
        // for the area to figure the width of the blocks
        evReg.each(Cal.calcMaxDepth);
     ...
     */
    this.checkConflictAtPos = function(i) {
        var evReg = Cal.eventRegistry;
     ...
     */
    this.calcConflictDepth = function(key, val) {
        var ev = val;
        var evReg = Cal.eventRegistry;
        var beforeEv = null;
        var depthList = [];
        ...
        ev.conflictDepth = Cal.findFirstGapInSequence(depthList);
    };
    /**
     * Find the first gap available for indenting an event
     */
    this.findFirstGapInSequence = function(depthList) {
        if (depthList.length) {
     ...
     */
    this.calcMaxDepth = function(key, val) {
        var ev = val;
        var max = 0;
        var maxBefore = Cal.getMaxDepth(ev.beforeConflicts);
        var maxAfter = Cal.getMaxDepth(ev.afterConflicts);
     ...
     */
    this.getMaxDepth = function(evArr) {
        var max = 0;
     ...
     */
    this.stackUntimed = function() {
        Cal.eventRegistry.each(Cal.setMatrixPos);
        return true;
    }
    /**
     *
     */
    this.setMatrixPos = function(key, val) {
        var ev = val;
        var startCol = 0;
        var endCol = 0;
        var evLength = 0;
        var matrix = Cal.allDayMatrix;
        var isPlaced = false;
        var row = 0;
        ...
        if (ev.data.allDay) {
            ...
            while (!isPlaced) {
                ...
                if (Cal.placeEventInRow(row, startCol, endCol)) {
                    isPlaced = true;
                    ev.allDayRow = row;
                    return true;
                }
     ...
     */
    this.placeEventInRow = function(row, startCol, endCol) {
        var matrix = Cal.allDayMatrix;
